-- ========================================================================
-- 阶段 2 已废弃：市场挂单与任务解耦后，新挂单只挂一个商品，多 item 任务不再生成。
--   - application 层 createTask 限制 contractJson.items.length === 1
--   - 挂单端点走 /listings，单 item
--   - 此脚本**仅用于历史数据**：在解耦前已发布的多 item 任务（id LIKE '%-d-%'）
--     仍然存在，可作为审计记录保留。
--   - 不要再跑此脚本做"清理"——listings 模块会按 commodity 拉市场挂单。
-- ========================================================================

-- 数据迁移：把现有多物品任务拆解成单物品任务。
-- （仅在 002 阶段 1 部署前执行过；现在不再需要）
--
-- 设计：
