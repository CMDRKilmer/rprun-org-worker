---
name: create-skill
description: Create a new Codex skill (slash command). Guides through designing the SKILL.md — the prompt file that teaches Codex how to perform a complex task. Triggers on "create skill", "new skill", "write a skill", "add a slash command".
---

# Create a Codex Skill

A Codex skill is a markdown file (SKILL.md) that teaches Codex how to perform a specific, repeatable task via a slash command. Skills live in `.Codex/skills/<skill-name>/SKILL.md`.

**What a skill IS:** A prompt — structured instructions that Codex reads and follows when the user invokes a slash command. It can be as simple as a checklist or as complex as a multi-phase interactive workflow.

**What a skill is NOT:** It is not code, not a plugin, not an API integration. It is structured prose that tells Codex what to do, what to ask, what order to do it in, and how to handle errors.
...
```bash
ls .Codex/skills/ 2>/dev/null || echo "NO_SKILLS_DIR"
```

If `.Codex/skills/` doesn't exist, create it. If skills already exist, read 2-3 of them to understand the project's conventions, tone, and structure. Match those conventions in the new skill. If this is the first skill in the project, the writing rules below are the convention baseline.

### Check for name collision

If the user has already suggested a skill name, check if `.Codex/skills/<name>/` already exists. If it does, ask whether to overwrite or choose a different name.
...
1. **Read the project's AGENTS.md** (if it exists) — understand the project structure, conventions, key files
2. **Identify the files** the skill will need to read or modify
3. **Read those files** to understand their current structure. A skill that tells Codex to modify `src/config.ts` must know what `src/config.ts` actually looks like. Extract exact code patterns — imports, exports, function signatures, switch cases — that the skill will use as find/replace anchors. Vague references like "the config file" break; exact `// Find:` / `// Replace with:` blocks succeed.
...
Create `.Codex/skills/<skill-name>/SKILL.md` following the structure below.
...
# Missing triggers — Codex won't know when to load it
...
- No XML angle brackets (`<` or `>`) anywhere in frontmatter — frontmatter appears in Codex's system prompt and could inject instructions
- Do not use "Codex" or "anthropic" in skill names — these are reserved
...
These are powerful — they override Codex's default tendencies (like asking too many questions or telling the user to run commands themselves). Common directives:

| Directive | When to use |
|-----------|-------------|
| `**Principle:** Handle everything automatically.` | Setup/integration skills — prevent Codex from just listing steps |
...
Use them when the skill needs a specific behavioral posture. Omit them for simple skills where Codex's defaults are fine.
...
Every sentence should be an action Codex can take. If it's not actionable, cut it.
...
For complex changes, use find/replace anchors so Codex knows exactly where to edit:
...
The blockquote is the message shown to the user. "Wait for..." tells Codex to pause.

#### 5. Commands are explicit — Codex runs them, not the user
...
This is one of the most important patterns. Real-world skills constantly branch on detected state, platform, command output, and user choices. Without clear conditional structure, Codex will either guess wrong or ask unnecessary questions.
...
**Progressive disclosure:** Keep SKILL.md focused on core instructions — move detailed reference material (API docs, long examples, style guides) to `references/`. Codex loads SKILL.md when it thinks the skill is relevant; linked files are only loaded on demand. Aim to keep SKILL.md under 5,000 words.
...
A skill that's too short will leave Codex guessing. A skill that's too long will bury the important instructions. When in doubt, err on the side of more detail for complex tasks and less for simple ones.

## Phase 5: Update Project Documentation

Read the project's `AGENTS.md` (or equivalent documentation file). If it has a section listing available skills or commands, add the new skill with a one-line description.
...
- [ ] Every instruction is an action Codex can take (no passive explanations)
...
- [ ] Commands are in fenced code blocks — Codex runs them directly
...
- [ ] Skill name does not contain "Codex" or "anthropic"
