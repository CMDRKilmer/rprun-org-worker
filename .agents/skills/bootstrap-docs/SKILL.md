...
List all directories under the project root, excluding common non-source directories: `node_modules`, `.git`, `dist`, `build`, `.next`, `target`, `__pycache__`, `.gradle`, `bin`, `obj`, `.Codex`, `docs`, `vendor`, `.venv`, `env`. Limit depth to 3 levels.
